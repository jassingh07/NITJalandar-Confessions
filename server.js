...
mongoose.connect(
  "mongodb+srv://"+ process.env.MONGO_USERNAME + ":" + process.env.MONGO_PASSWORD +"@cluster0.rhrrq3r.mongodb.net/ConfessionDB",
...
passport.use(
  new GoogleStrategy(
    ...
    function (accessToken, refreshToken, profile, cb) {
      ...
      // if (profile._json.hd === "nitj.ac.in") {
      ...
      // } else {
      //   // fail
      //   res.send("Acess restricted to organization only");
...
app
  ...
  .post((req, res) => {
    ...
    User.findOne({ username: req.body.username })
      ...
      .catch((err) => {
        res.send(
          "Database Error: Unable to check credentials, plz try again!!"
        );
      });
  });
